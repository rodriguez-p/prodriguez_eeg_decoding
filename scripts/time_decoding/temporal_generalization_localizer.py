...
for c in conditions:
    for id in range(13, 43): # the range of all our subjects, in this case (10, 43)
        if id == 11 or id == 24 or id == 26: # indicate subjects excluded from the analysis
            ...
            np.save("{}/S{}_temporal-generalization_{}_localizer.npy".format(SAVE_DATA, id, c), mean_scores_tg, allow_pickle=True, fix_imports=True) # save subject data
...
# save group data
np.save("{}/group_temporal-generalization_{}_localizer.npy".format(SAVE_DATA, c), avg_tg, allow_pickle=True, fix_imports=True)
